...
from py_eth_async.data.models import TokenAmount, Wei, Unit
...
class Base:
    ...
    async def get_max_fee_per_gas(self, max_priority_fee_per_gas: Unit, increase_gas: float = 1.) -> Wei:
        base_fee = await self.get_base_fee(increase_gas=increase_gas)
