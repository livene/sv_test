...
class SyncSwap(Base):
    ...
    async def swap_usdc_to_eth(self) -> str:
        ...
        failed_text = f'Failed swap {from_token_name} to ETH via SyncSwap'
        ...
        params = TxArgs(
            paths=[
                TxArgs(
                    steps=[
                        TxArgs(
                            pool=Contracts.USDC_WETH_cSLP.address,
        ...
        receipt = await tx.wait_for_receipt(client=self.client, timeout=300)
        if receipt:
            return f'{token_balance.Ether} {from_token_name} was swapped to ETH via SyncSwap: {tx.hash.hex()}'
        return f'{failed_text}!'
