...
class MySwapLiquidity(Base):
    ...
    async def add_liquidity(self, token_out_name: str, slippage: float = 2.):
        ...
        try:
            ...
            token_approve_call = token_contract.functions['approve'].prepare(
                spender=models.MY_SWAP_ROUTER,
                amount=token_amount.Wei
            )
            eth_approve_call = eth_contract.functions['approve'].prepare(
                spender=models.MY_SWAP_ROUTER,
                amount=eth_amount.Wei
            )

            add_liquidity_call = myswap_contract.functions['add_liquidity'].prepare(
    ...
    async def remove_liquidity(self, token_out_name: str, slippage: float = 1):
        ...
        try:
            ...
            approve_call = lptoken_contract.functions['approve'].prepare(
                spender=models.MY_SWAP_ROUTER,
                amount=amount_in.Wei
            )
            withdraw_liquidity_call = myswap_contract.functions['withdraw_liquidity'].prepare(
                pool_id=pool_id,
