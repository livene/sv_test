...
class Testnetbridge(Base):
    ...
    async def action(self):
        ...
        amount = TokenAmount(
            amount=randfloat(
                from_=settings.testnetbridge_swaps_amount.from_,
                to_=settings.testnetbridge_swaps_amount.to_,
                step=0.0000001
            ),
            decimals=await self.get_decimals(contract_address=Contracts.ARBITRUM_GETH.address)
        )

        return await self.swap_geth(
            geth_amount=amount,
