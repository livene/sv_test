...
class CoredaoBridge(Base):
    ...
    async def action(self):
        settings = Settings()
        self.client = Client(private_key=self.client.account.key, network=Networks.BSC, proxy=self.client.proxy)

        amount = TokenAmount(
            amount=randfloat(
                from_=settings.coredao_swaps_amount.from_,
                to_=settings.coredao_swaps_amount.to_,
                step=0.0000001
            ),
            decimals=await self.get_decimals(contract_address=Contracts.BSC_USDT.address)
        )

        return await self.bridge_usdt_bsc_to_usdt_coredao(
            amount=amount,
