...
from lesson_3.data.config import logger
...
class Base:
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(f'https://api.binance.com/api/v3/depth?limit=1&symbol={token}USDT') as r:
                        ...
                        if 'asks' not in result_dict:
                            ...
                            exit()
                        logger.info(result_dict['asks'][0][0])
    ...
    async def tokenprice_to_another_tokenprice(self, tokenfirst: 'ETH', tokensecond: 'BTC', retries: int = 5):
        tokenfirst = tokenfirst.upper()
        tokensecond = tokensecond.upper()

        for _ in range(retries):
            try:
                async with aiohttp.ClientSession() as session:
                    logger.info(
                        f'{self.client.account.address} | getting {tokenfirst} price to {tokensecond}')
                    async with session.get(f'https://api.binance.com/api/v3/depth?limit=1&symbol={tokenfirst}{tokensecond}') as r:
                        if r.status != 200:
                            logger.error(f'code: {r.status} | json: {r.json()}')
                            exit()
                        result_dict = await r.json()
                ...
                logger.error(
                    f'{self.client.account.address} | getting {tokenfirst} price to {tokensecond}: {e}')
                await asyncio.sleep(5)
        exit()
